...
assert WORD.lower().count('r') == 3 and len(WORD) == 10
out = Path(os.environ['JEV_ROOT']) / 'results/counting-strawberry-v1'
...
messages = [
    ...
    {'role': 'user', 'content': 'How many R are in the exact word "strawberry"? Count case-insensitively (R and r are the same letter). Do not correct the spelling.\nOptions:\n' + '\n'.join(f'{chr(65+i)}. {i}' for i in OPTIONS)}
]
